...
export interface IOrderRepository {
    ...
    getAll(page: number, limit: number, status: string, search: string): Promise<IOrderListResult>;
...
export class OrderMongoRepository implements IOrderRepository {
    ...
    async getAll(page: number, limit: number, status: string, search: string): Promise<IOrderListResult> {
        const query: any = {};
        if (status) query.status = status;
        if (search) {
            query.$or = [
